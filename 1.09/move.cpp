...
void do_combat(dungeon_t *d, character *atk, character *def) {
    //Attack only if the pc is involved
    if (atk == d->thepc || def == d->thepc) {
        uint atk_damage = atk->damage->roll();
        if (atk == d->thepc) {
            atk_damage += d->thepc->damage_bonus();
        }
...
void move_character(dungeon_t *d, character *c, pair_t next) {
    if (charpair(next) &&
        ...
        if (d->charmap[next[dim_y]][next[dim_x]] != NULL) {
            pair_t pos;
            pos[dim_y] = next[dim_y];
            pos[dim_x] = next[dim_x];
            int i;
            for (i = 0; i < 8; i++) {
                if (i == 0) {
                    if (d->map[pos[dim_y] - 1][pos[dim_x] + 1] >= ter_floor) {
                        if (d->charmap[pos[dim_y] - 1][pos[dim_x] + 1]) {
                            if (pos[dim_y] - 1 > 1) {
                                if (pos[dim_x] + 1 < DUNGEON_X) {
                                    if (pos[dim_y - 1] == d->thepc->position[dim_y] &&
                                        pos[dim_x + 1] == d->thepc->position[dim_x]) {
                                        d->charmap[character_get_y(c)][character_get_x(c)] = NULL;
                                        character_set_y(c, pos[dim_y] - 1);
                                        character_set_x(c, pos[dim_x] + 1);
                    ...
                    if (d->map[pos[dim_y]][pos[dim_x] + 1] >= ter_floor) {
                        if (d->charmap[pos[dim_y]][pos[dim_x] + 1]) {
                            if (pos[dim_x] + 1 < DUNGEON_X) {
                                if (pos[dim_y] == d->thepc->position[dim_y] &&
                                    pos[dim_x + 1] == d->thepc->position[dim_x]) {
                                    d->charmap[character_get_y(c)][character_get_x(c)] = NULL;
                                    character_set_y(c, pos[dim_y]);
                                    character_set_x(c, pos[dim_x] + 1);
                                    d->charmap[character_get_y(c)][character_get_x(c)] = c;
                                    break;
                                }
                    ...
                    if (d->map[pos[dim_y] + 1][pos[dim_x]] >= ter_floor) {
                        if (d->charmap[pos[dim_y] + 1][pos[dim_x]]) {
                            if (pos[dim_y] + 1 > DUNGEON_Y) {
                                if (pos[dim_y + 1] == d->thepc->position[dim_y] &&
                                    pos[dim_x] == d->thepc->position[dim_x]) {
                                    d->charmap[character_get_y(c)][character_get_x(c)] = NULL;
                                    character_set_y(c, pos[dim_y] + 1);
                                    character_set_x(c, pos[dim_x]);
                                    d->charmap[character_get_y(c)][character_get_x(c)] = c;
                                    break;
                                }
                            }
