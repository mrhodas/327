...
void io_display(dungeon_t *d) {
    ...
    for (y = 0; y < DUNGEON_Y; y++) {
        for (x = 0; x < DUNGEON_X; x++) {
            ...
            } else if (d->itemmap[y][x] &&
                       can_see(d,
                               character_get_pos(d->thepc),
                               d->itemmap[y][x]->position,
                               1)) {
                if (d->itemmap[y][x]->color && d->itemmap[y][x]->symbol) {
                    attron(COLOR_PAIR(d->itemmap[y][x]->color));
                    mvaddch(y + 1, x, d->itemmap[y][x]->symbol);
                    attroff(COLOR_PAIR(d->itemmap[y][x]->color));
                }
            } else {
