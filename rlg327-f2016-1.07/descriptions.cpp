...
void gen_monsters(dungeon_t *d) {
    ...
    for (i = 0; i < d->num_monsters; i++) {
        ...
        int monster_id = rand_range(0, d->monster_descriptions.size() - 1);

        monster_description des = d->monster_descriptions[monster_id];

        m->position[dim_y] = p[dim_y];
        m->position[dim_x] = p[dim_x];
        d->charmap[p[dim_y]][p[dim_x]] = m;

        m->speed = des.getSpeed().roll();
        m->alive = 1;
        m->sequence_number = ++d->character_sequence_number;

        m->characteristics = des.getAbilities();
        m->symbol = des.getSymbol();

        m->have_seen_pc = 0;
        m->pc_last_known_position[dim_y] = 0;
        m->pc_last_known_position[dim_x] = 0;

        m->set(des.getName(), des.getDescription(), des.getColor(), des.getDamage(), des.getHitpoints());
    ...
    for (i = 0; i < 10; i++) {
        ...
        do {
            ...
            p[dim_x] = rand_range(d->rooms[room].position[dim_x],
                                  (d->rooms[room].position[dim_x] +
                                   d->rooms[room].size[dim_x] - 1));
        } while (d->itemmap[p[dim_y]][p[dim_x]]);

        int object_id = rand_range(0, d->object_descriptions.size() - 1);

        object_description des = d->object_descriptions[object_id];

        m->position[dim_y] = p[dim_y];
        m->position[dim_x] = p[dim_x];
        d->itemmap[p[dim_y]][p[dim_x]] = m;
        m->type = des.getType();
        m->color = des.getColor();
        m->hit_bonus = des.getHit().roll();
        m->damage_bonus = des.getDamage().roll();
        m->dodge_bonus = des.getDodge().roll();
        m->weight = des.getWeight().roll();
        m->speed_bonus = des.getSpeed().roll();
        m->special = des.getAttribute().roll();
        m->val = des.getValue();
        m->symbol = itemChars[m->type];
...
const std::string &monster_description::getName() const {
    return name;
}

const std::string &monster_description::getDescription() const {
    return description;
}

char monster_description::getSymbol() const {
    return symbol;
}

const std::vector<uint32_t> &monster_description::getColor() const {
    return color;
}

uint32_t monster_description::getAbilities() const {
    return abilities;
}

const dice &monster_description::getSpeed() const {
    return speed;
}

const dice &monster_description::getHitpoints() const {
    return hitpoints;
}

const dice &monster_description::getDamage() const {
    return damage;
}
...
const std::string &object_description::getName() const {
    return name;
}

const std::string &object_description::getDescription() const {
    return description;
}

object_type_t object_description::getType() const {
    return type;
}

uint32_t object_description::getColor() const {
    return color;
}

const dice &object_description::getHit() const {
    return hit;
}

const dice &object_description::getDamage() const {
    return damage;
}

const dice &object_description::getDodge() const {
    return dodge;
}

const dice &object_description::getDefence() const {
    return defence;
}

const dice &object_description::getWeight() const {
    return weight;
}

const dice &object_description::getSpeed() const {
    return speed;
}

const dice &object_description::getAttribute() const {
    return attribute;
}

const dice &object_description::getValue() const {
    return value;
}
